...
import { parsePTCGO } from '../lib/pokemon-shared/ptcgoParser';
...
export function importFromPTCGO(ptcgoText: string, cards: Map<string, Card>): ImportResult {
  const output = parsePTCGO(ptcgoText, cards.values());
  return {
    success: output.errors.length === 0,
    imported: output.entries.reduce((n, e) => n + e.quantity, 0),
    skipped: output.errors.length,
    errors: output.errors.map(e => e.message),
    detailedErrors: output.errors.map(e => ({ line: e.line, cardName: e.cardName, message: e.message, type: 'validation' as const })),
    cards: Object.fromEntries(output.entries.map(e => [e.cardId, e.quantity])),
  };
}
